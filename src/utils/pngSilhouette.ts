...
export function extractPNGSilhouetteContour(
  ...
  try {
    ...
    const gridW = 100;
    const gridH = Math.max(20, Math.round(100 * (targetHeight / Math.max(1, targetWidth))));
    canvas.width = gridW;
    canvas.height = gridH;
    ...
    // Build binary mask where pixel is solid if alpha > 25
    const mask: boolean[][] = [];
    let solidCount = 0;
    ...
    // Downsample contour to a clean set of smooth points (~30-50 points)
    const targetPointCount = Math.min(48, Math.max(16, Math.floor(boundaryPoints.length / 2)));
